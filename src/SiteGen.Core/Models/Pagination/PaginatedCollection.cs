...
public class PaginatedCollection<T> : List<T>, IPaginatedCollection<T>
{
    ...
    /// </summary>
    public PaginatedCollection(IEnumerable<T> source) : this(source, 1, defaultPageSize)
    {
    }
    ...
    public PaginatedCollection(IEnumerable<T> source, int page, int pageSize, int recordCount)
    {
        ...
        // Default page names
        pages = Enumerable.Range(1, PageCount).Select(x => x.ToString(CultureInfo.CurrentCulture)).ToList();
